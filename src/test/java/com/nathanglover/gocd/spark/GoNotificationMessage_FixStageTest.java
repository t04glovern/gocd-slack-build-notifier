...
public class GoNotificationMessage_FixStageTest {
    ...
    public GoNotificationMessage_FixStageTest(History pipelineHistory,
        GoNotificationMessage.PipelineInfo pipeline, String expectedStatus) {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][]{
            // One history pipeline, same pipeline run
            {
                ...
                whenPipelineFinished(pipeline(PIPELINE_NAME, counter(1),
                    stage(STAGE_NAME, counter(2), Status.Building))),
                thenExpectStatus(Status.Building)
            }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Failed))),
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(2), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Failed))),
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(2), stage(STAGE_NAME, counter(2), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed))),
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(2), Status.Failed))),
            ...
            givenHistory(pipeline(PIPELINE_NAME, counter(1),
                stage(STAGE_NAME, counter(1), Status.Cancelled))),
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(2), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        }, {
            ...
            givenHistory(pipeline(PIPELINE_NAME, counter(1),
                stage(STAGE_NAME, counter(1), Status.Cancelled))),
            whenPipelineFinished(pipeline(PIPELINE_NAME, counter(1),
                stage(STAGE_NAME, counter(2), Status.Cancelled))),
            ...
            // Multiple stages
            {
                ...
                whenPipelineFinished(pipeline(PIPELINE_NAME, counter(1),
                    stage(STAGE_NAME, counter(4), Status.Passed))),
                thenExpectStatus(Status.Fixed)
            }, {
            givenHistory(pipeline(PIPELINE_NAME, counter(1),
            ...
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(4), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        },
            ...
            {
                givenHistory(pipeline(PIPELINE_NAME, counter(1),
                    stage(STAGE_NAME, counter(1), Status.Passed))),
                whenPipelineFinished(pipeline(PIPELINE_NAME, counter(2),
                    stage(STAGE_NAME, counter(1), Status.Failed))),
                thenExpectStatus(Status.Broken)
            }, {
            givenHistory(
            ...
            givenHistory(noPipelines()),
            whenPipelineFinished(pipeline(PIPELINE_NAME, counter(1),
                stage(STAGE_NAME, counter(1), Status.Building))),
            thenExpectStatus(Status.Building)
        },
            // Longer history, next pipeline run
            {
                givenHistory(pipeline(PIPELINE_NAME, counter(1),
                    stage(STAGE_NAME, counter(1), Status.Failed)),
                    pipeline(PIPELINE_NAME, counter(2),
                        stage(STAGE_NAME, counter(1), Status.Failed)),
                    pipeline(PIPELINE_NAME, counter(3),
                        stage(STAGE_NAME, counter(1), Status.Failed))),
                whenPipelineFinished(pipeline(PIPELINE_NAME, counter(4),
                    stage(STAGE_NAME, counter(1), Status.Passed))),
                thenExpectStatus(Status.Fixed)
            }, {
            givenHistory(
            ...
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(4), stage(STAGE_NAME, counter(1), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
            ...
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(4), stage(STAGE_NAME, counter(1), Status.Failed))),
            thenExpectStatus(Status.Broken)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
            ...
            {
                givenHistory(
                    pipeline(PIPELINE_NAME, counter(1),
                        stage(STAGE_NAME, counter(1), Status.Passed)),
                    pipeline(PIPELINE_NAME, counter(2),
                        stage(STAGE_NAME, counter(1), Status.Passed)),
                    pipeline(PIPELINE_NAME, counter(3),
                        stage(STAGE_NAME, counter(1), Status.Passed)),
                    pipeline(PIPELINE_NAME, counter(3),
                        stage(STAGE_NAME, counter(2), Status.Passed))
                ),
                whenPipelineFinished(pipeline(PIPELINE_NAME, counter(3),
                    stage(STAGE_NAME, counter(3), Status.Failed))),
                thenExpectStatus(Status.Broken)
            }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
            ...
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(3), stage(STAGE_NAME, counter(3), Status.Passed))),
            thenExpectStatus(Status.Fixed)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
            ...
            whenPipelineFinished(
                pipeline(PIPELINE_NAME, counter(3), stage(STAGE_NAME, counter(3), Status.Failed))),
            thenExpectStatus(Status.Failed)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
            ...
            whenPipelineFinished(pipeline(PIPELINE_NAME, counter(3),
                stage(STAGE_NAME, counter(3), Status.Cancelled))),
            thenExpectStatus(Status.Cancelled)
        }, {
            givenHistory(
                pipeline(PIPELINE_NAME, counter(1), stage(STAGE_NAME, counter(1), Status.Passed)),
