...
public class GoNotificationPlugin extends AbstractNotificationPlugin implements GoPlugin {
    ...
    public static final String HOME_PLUGIN_CONFIG_PATH =
            ...
            public void run() {
                if (pluginConfig.lastModified() != configLastModified) {
                    if (configLastModified == 0l) {
                        ...
                        LOGGER.info(
                            "Reloading configuration file since some modifications were found");
                    }
                    try {
                        lock.writeLock().lock();
    ...
    private GoPluginApiResponse handleRequestGetView() {
        ...
        try {
            String template = IOUtils
                .toString(getClass().getResourceAsStream("/views/config.template.html"), "UTF-8");
    ...
    private GoPluginApiResponse handleRequestGetConfiguration() {
        Map<String, Object> response = new HashMap<String, Object>();
        response.put("server-url-external",
            configField("External GoCD Server URL", "", "1", true, false));
        response.put("pipelineConfig",
            configField("Pipeline Notification Rules", "", "2", true, false));
        return renderJSON(SUCCESS_RESPONSE_CODE, response);
    }
    ...
    private GoPluginApiResponse handleStageNotification(GoPluginApiRequest goPluginApiRequest) {
        ...
        try {
            ...
            LOGGER.info(
                message.fullyQualifiedJobName() + " has " + message.getStageState() + "/" + message
                    .getStageResult());
            lock.readLock().lock();
    ...
    private GoNotificationMessage parseNotificationMessage(GoPluginApiRequest goPluginApiRequest) {
        return new GsonBuilder().create()
            .fromJson(goPluginApiRequest.requestBody(), GoNotificationMessage.class);
    }
    ...
    private File findGoNotifyConfigPath() {
        ...
        if (StringUtils.isNotEmpty(goNotifyConfPath)) {
            ...
            if (pluginConfig.exists()) {
                LOGGER.info(String.format("Configuration file found using GO_NOTIFY_CONF at %s",
        ...
        if (pluginConfig.exists()) {
            LOGGER.info(String.format("Configuration file found at Home Dir as %s",
                pluginConfig.getAbsolutePath()));
            return pluginConfig;
        }
        ...
        if (pluginConfig.exists()) {
            LOGGER.info(String.format("Configuration file found using CRUISE_SERVER_DIR at %s",
                pluginConfig.getAbsolutePath()));
            return pluginConfig;
        }

        throw new RuntimeException(
            "Unable to find go_notify.conf. Please make sure you've set it up right.");
    }
}
