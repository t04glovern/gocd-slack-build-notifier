...
public class SparkPipelineListener extends PipelineListener {
	...
	private String sparkAttachment(PipelineRule rule, GoNotificationMessage message, PipelineStatus pipelineStatus) throws URISyntaxException {
		...
		if (rules.getDisplayMaterialChanges()) {
			try {
				...
				for (MaterialRevision change : changes) {
					...
					if (rules.isTruncateChanges() && change.modifications.size() > DEFAULT_MAX_CHANGES_PER_MATERIAL_IN_SPARK) {
						change.modifications = Lists.take(change.modifications, DEFAULT_MAX_CHANGES_PER_MATERIAL_IN_SPARK);
						isTruncated = true;
					}
					...
					String fieldNamePrefix = (isTruncated) ? String.format("Latest %d", DEFAULT_MAX_CHANGES_PER_MATERIAL_IN_SPARK) : "All";
